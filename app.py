...
def process_files(task_id: str, zeus_path: Path, keytrak_path: Path, recipient: str) -> None:
    ...
    try:
        ...
        # Clean up uploaded input files right after processing.
...
def upload():
    ...
    thread = threading.Thread(target=process_files, args=(task_id, zeus_path, keytrak_path, recipient), daemon=True)
    thread.start()
